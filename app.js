...
//   });
  ...
  const getWebsitedotproperty = async (keyword) => {
        ...
        const exportResults = (parsedResults) => {
                ...
                fs.writeFile("dotproperty.pug", parsedResults.toString(), (err) => {
                    if (err)
                      console.log(err);
                    else {
                     console.log(chalk.yellow.bgBlue(`File written successfully\n${parsedResults.length} รายการ dotproperty\n`))
                     sendstatus(parsedResults.length)
                    }
                  });
                }

                function sendstatus(length){
                    fs.writeFile("views/rscon.pug",`File written successfully\n${length} รายการ dotproperty\n<a href="/condo"><button type="submit">ดูคอนโด</button><a>`, (err) => {
                        if (err)
                          console.log(err);
                        else {
                            console.log("ss")
